...
pub trait StringExt {
    ...
    fn tag_name(&self) -> Option<Tag> {
        self.as_tag().map(|(_, name)| name)
    }
    fn tag_direction(&self) -> Option<TagDirection> {
        self.as_tag().map(|(dir, _)| dir)
    }
...
mod tests {
    ...
    fn test_as_tag() {
        ...
        assert_eq!(
            "-+foo".as_tag(),
            Some((TagDirection::Removed, "+foo".into()))
        );
